...
function DetailPanel() {
  ...
  const { items, selectedId, content, publications, publishingAccounts, jobs, isLoadingDetail, isSaving, detailError, conflict, select, savePlanItem, saveContent, approveContent, schedulePublication, createJob, refresh, clearConflict } = useContentStore();
  ...
  const [scheduleDraft, setScheduleDraft] = useState({ accountId: '', desiredScheduledAt: '', externalUrl: '', copy: '' });
  ...
  return <div className="fixed inset-0 z-40 flex justify-end bg-slate-950/30" role="dialog" aria-modal="true" aria-label={`Detalle de ${item.title}`} onMouseDown={(event) => { if (event.currentTarget === event.target) void select(token, null); }}><aside className="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl">
    ...
    <div className="space-y-5 p-5">{conflict && <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><div className="flex gap-2"><AlertCircle className="size-5 shrink-0" /><div><p className="font-bold">Hay una versión más reciente</p><p className="mt-1">{conflict}</p><Button className="mt-3 bg-amber-900 text-white" onClick={() => { clearConflict(); void refresh(token); }}>Recargar datos</Button></div></div></div>}{detailError && <p role="alert" className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{detailError}</p>}{isLoadingDetail && <div className="flex items-center gap-2 text-sm text-slate-500"><LoaderCircle className="size-4 animate-spin" />Cargando detalle…</div>}
      {tab === 'brief' && <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (!admin) return; const splitList = (value: string) => value.split(',').map((entry) => entry.trim()).filter(Boolean); void savePlanItem(token, item.id, { ...draft, keywords: splitList(draft.keywords), entities: splitList(draft.entities), plannedAt: draft.plannedAt ? new Date(draft.plannedAt).toISOString() : null, version: item.version }); }}><Field label="Título" value={draft.title} onChange={(title) => setDraft({ ...draft, title })} disabled={!admin} /><Field label="Tema" value={draft.theme} onChange={(theme) => setDraft({ ...draft, theme })} disabled={!admin} /><Field label="Justificación" value={draft.rationale} onChange={(rationale) => setDraft({ ...draft, rationale })} disabled={!admin} multiline /><div className="grid gap-3 sm:grid-cols-2"><Field label="Formato" value={draft.format} onChange={(formatValue) => setDraft({ ...draft, format: formatValue })} disabled={!admin} /><Field label="Palabra clave" value={draft.keywordPrimary} onChange={(keywordPrimary) => setDraft({ ...draft, keywordPrimary })} disabled={!admin} /><Field label="Keywords (separadas por comas)" value={draft.keywords} onChange={(keywords) => setDraft({ ...draft, keywords })} disabled={!admin} /><Field label="Entidades (separadas por comas)" value={draft.entities} onChange={(entities) => setDraft({ ...draft, entities })} disabled={!admin} /><Field label="Fecha editorial" type="datetime-local" value={draft.plannedAt} onChange={(plannedAt) => setDraft({ ...draft, plannedAt })} disabled={!admin} /><Field label="CTA" value={draft.cta} onChange={(cta) => setDraft({ ...draft, cta })} disabled={!admin} /></div>{admin && <div className="flex flex-wrap gap-2"><Button type="submit" disabled={isSaving} className="bg-brand-primary text-white">Guardar cambios</Button>{item.status === 'proposed' && <Button type="button" disabled={isSaving} onClick={() => void savePlanItem(token, item.id, { status: 'approved', version: item.version })} className="bg-emerald-600 text-white">Aprobar propuesta</Button>}<Button type="button" disabled={isSaving || !['approved', 'review', 'ready', 'generation_failed'].includes(item.status)} onClick={() => void createJob(token, { clientId: item.clientId, kind: 'generate_content', targetId: item.id, expectedVersion: item.version })} className="bg-slate-900 text-white"><Sparkles className="size-4" />Generar contenido</Button>{['proposed', 'approved', 'review', 'ready', 'generation_failed'].includes(item.status) && <Button type="button" disabled={isSaving} onClick={() => { if (!window.confirm('¿Eliminar esta propuesta? Dejará de aparecer en el calendario.')) return; void savePlanItem(token, item.id, { status: 'archived', version: item.version }); }} className="border border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100"><Trash2 className="size-4" />Eliminar</Button>}</div>}</form>}
      ...
      {tab === 'publications' && <div className="space-y-4">{admin && content?.status === 'approved' && content.approvedRevisionId && <form className="space-y-3 rounded-xl border border-indigo-200 bg-indigo-50/50 p-4" onSubmit={(event) => { event.preventDefault(); if (!scheduleDraft.accountId || !scheduleDraft.desiredScheduledAt || (requiresActionUrl && !scheduleDraft.externalUrl.trim())) return; void schedulePublication(token, { contentId: content.id, clientId: content.clientId, expectedVersion: content.version, accountId: scheduleDraft.accountId, desiredScheduledAt: new Date(scheduleDraft.desiredScheduledAt).toISOString(), externalUrl: scheduleDraft.externalUrl.trim() || undefined, copy: scheduleDraft.copy || undefined }); }}><div><h3 className="font-bold text-slate-900">Programar publicación</h3><p className="mt-1 text-xs text-slate-500">Se enviará la revisión aprobada y quedará vinculada a esta cuenta.</p></div><label className="block text-xs font-bold text-slate-500">Cuenta<select required value={scheduleDraft.accountId} onChange={(event) => setScheduleDraft({ ...scheduleDraft, accountId: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="">Selecciona una cuenta</option>{publishingAccounts.map((account) => <option key={account.id} value={account.id}>{account.label} · {account.provider}{account.platform ? ` · ${account.platform}` : ''}</option>)}</select></label><Field label="Fecha de publicación" type="datetime-local" value={scheduleDraft.desiredScheduledAt} onChange={(desiredScheduledAt) => setScheduleDraft({ ...scheduleDraft, desiredScheduledAt })} /><Field label={requiresActionUrl ? "URL obligatoria para CTA de Google" : "URL del enlace (opcional)"} type="url" value={scheduleDraft.externalUrl} onChange={(externalUrl) => setScheduleDraft({ ...scheduleDraft, externalUrl })} />{requiresActionUrl && <p className="text-xs text-amber-700">Google Business Profile necesita una URL de destino para la accion Mas informacion.</p>}<Field label="Texto adaptado (opcional)" value={scheduleDraft.copy} onChange={(copy) => setScheduleDraft({ ...scheduleDraft, copy })} multiline rows={4} /><Button type="submit" disabled={isSaving || !scheduleDraft.accountId || !scheduleDraft.desiredScheduledAt || (requiresActionUrl && !scheduleDraft.externalUrl.trim())} className="bg-indigo-600 text-white"><Send className="size-4" />Programar</Button>{!publishingAccounts.length && <p className="text-xs font-semibold text-amber-700">No hay cuentas de publicación activas para este cliente.</p>}</form>}{publications.length ? <div className="space-y-3">{publications.map((publication) => <article key={publication.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-slate-800">{publication.accountLabel}</h3><p className="text-xs text-slate-500">{publication.provider}{publication.platform ? ` · ${publication.platform}` : ''}</p></div><ContentStatusBadge status={publication.status} /></div><dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="font-bold text-slate-400">Fecha deseada</dt><dd className="mt-1 text-slate-700">{formatEditorialDate(publication.desiredScheduledAt)}</dd></div><div><dt className="font-bold text-slate-400">Fecha confirmada</dt><dd className="mt-1 text-slate-700">{formatEditorialDate(publication.confirmedScheduledAt)}</dd></div></dl>{publication.errorMessage && <p className="mt-3 rounded-lg bg-rose-50 p-2 text-xs text-rose-700">{publication.errorMessage}</p>}{publication.externalUrl && <a className="mt-3 inline-block text-xs font-bold text-brand-primary hover:underline" href={publication.externalUrl} target="_blank" rel="noreferrer">Abrir publicación</a>}{admin && ['failed', 'unknown'].includes(publication.status) && <Button onClick={() => void createJob(token, { clientId: publication.clientId, kind: publication.status === 'unknown' ? 'reconcile' : 'publish', targetId: publication.id, expectedVersion: publication.version })} className="mt-3 bg-slate-900 text-white">{publication.status === 'unknown' ? 'Comprobar estado' : 'Reintentar'}</Button>}{admin && canCancelPublication(publication.status) && <Button onClick={() => { if (!window.confirm('Solicitar cancelacion de esta publicacion?')) return; void createJob(token, { clientId: publication.clientId, kind: 'cancel', targetId: publication.id, expectedVersion: publication.version }).then(() => void select(token, item)); }} className="mt-3 ml-2 border border-rose-200 bg-rose-50 text-rose-700">Cancelar</Button>}{admin && canReschedulePublication(publication.status) && <><Button onClick={() => { setRescheduleTarget(rescheduleTarget === publication.id ? null : publication.id); setRescheduleAt(publication.desiredScheduledAt ? format(parseISO(publication.desiredScheduledAt), "yyyy-MM-dd'T'HH:mm") : ''); }} className="mt-3 ml-2 bg-indigo-50 text-indigo-700">Reprogramar</Button>{rescheduleTarget === publication.id && <form className="mt-3 flex flex-wrap items-end gap-2" onSubmit={(event) => { event.preventDefault(); if (!rescheduleAt) return; void createJob(token, { clientId: publication.clientId, kind: 'reschedule', targetId: publication.id, expectedVersion: publication.version, payload: { desiredScheduledAt: new Date(rescheduleAt).toISOString() } }).then(() => { setRescheduleTarget(null); void select(token, item); }); }}><label className="text-xs font-bold text-slate-500">Nueva fecha<input required type="datetime-local" value={rescheduleAt} onChange={(event) => setRescheduleAt(event.target.value)} className="mt-1 block rounded-xl border border-slate-200 px-3 py-2 text-sm" /></label><Button type="submit" disabled={isSaving || !rescheduleAt} className="bg-indigo-600 text-white">Confirmar fecha</Button></form>}</>}</article>)}</div> : <Empty label={content?.status === 'approved' ? 'Todavía no hay salidas de publicación' : 'Aprueba una revisión antes de programarla'} />}</div>}
      {tab === 'history' && <Timeline content={content} publications={publications} jobs={jobs} item={item} />}
    </div></aside></div>;
}

function Timeline({ content, publications, jobs, item }: { content: ReturnType<typeof useContentStore.getState>['content']; publications: ReturnType<typeof useContentStore.getState>['publications']; jobs: ReturnType<typeof useContentStore.getState>['jobs']; item: PlanItem }) {
...
function CreateCalendarPanel({ clientId, onClose }: { clientId: string; onClose: () => void }) {
  const token = useClientStore((state) => state.sessionToken)!;
  const createCalendar = useContentStore((state) => state.createCalendar);
  const isSaving = useContentStore((state) => state.isSaving);
  const [title, setTitle] = useState('');
  const [error, setError] = useState<string | null>(null);
  return <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/30 p-4" role="dialog" aria-modal="true" aria-label="Nuevo calendario editorial">
    <form className="w-full max-w-md space-y-4 rounded-2xl bg-white p-6 shadow-2xl" onSubmit={async (event) => {
      event.preventDefault();
      setError(null);
      try { await createCalendar(token, clientId, { title: title.trim() }); onClose(); }
      catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo crear el calendario'); }
    }}>
      <div className="flex items-center justify-between"><h2 className="text-xl font-bold">Nuevo calendario</h2><Button type="button" onClick={onClose} className="bg-slate-100 px-3 text-slate-700" aria-label="Cerrar"><X className="size-4" /></Button></div>
      <Field label="Título del calendario" value={title} onChange={setTitle} />
      {error && <p role="alert" className="text-sm text-rose-700">{error}</p>}
      <Button type="submit" disabled={isSaving || !title.trim()} className="w-full bg-brand-primary text-white">Crear calendario</Button>
    </form>
  </div>;
}
...
export function ContentTab({ clientId }: { clientId?: string | null }) {
  const token = useClientStore((state) => state.sessionToken)!;
  const role = useClientStore((state) => state.currentUser?.role);
  const { month, view, filters, items, nextCursor, isLoading, isLoadingMore, isRefreshing, error, lastUpdatedAt, jobs, setView, setMonth, reset, setFilters, load, loadMore, refresh, pollJobs, createJob } = useContentStore();
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-brand-primary">Planificación editorial</p><h1 className="mt-1 text-3xl font-bold text-slate-900">Contenidos</h1><p className="mt-2 text-sm text-slate-500">{clientId ? 'Calendario, revisiones y publicaciones del cliente.' : 'Vista global de todos los clientes.'}</p></div>{admin && <div className="flex flex-wrap gap-2"><Button onClick={() => setCreating(true)} className="bg-white text-slate-700 shadow-sm ring-1 ring-slate-200"><Plus className="size-4" />Nuevo contenido</Button><Button disabled={!filters.clientId || isRefreshing || isGeneratingPlan} onClick={() => void generatePlan()} className="bg-slate-900 text-white">{isGeneratingPlan ? <LoaderCircle className="size-4 animate-spin" /> : <Sparkles className="size-4" />}Generar plan</Button></div>}</div>
    ...
    {isLoading ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }, (_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl bg-slate-200" />)}</div> : filtered.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white"><Empty label={items.length ? 'Ningún contenido coincide con los filtros' : `No hay contenidos en ${monthLabel(month)}`} /></div> : <>{view === 'calendar' ? <><EditorialCalendar items={dated} />{nextCursor && <Button onClick={() => void loadMore(token)} disabled={isLoadingMore} className="bg-white text-slate-700 shadow-sm">{isLoadingMore && <LoaderCircle className="size-4 animate-spin" />}Cargar más contenidos del calendario</Button>}</> : <ContentTable items={filtered} />}<UndatedTray items={undated} /></>}
