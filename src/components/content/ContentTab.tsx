...
function DetailPanel() {
  ...
  const [scheduleDraft, setScheduleDraft] = useState({ accountId: '', desiredScheduledAt: '', copy: '' });
  ...
  useEffect(() => { if (content) setScheduleDraft({ accountId: '', desiredScheduledAt: item?.plannedAt ? format(parseISO(item.plannedAt), "yyyy-MM-dd'T'HH:mm") : '', copy: content.excerpt ?? '' }); }, [content, item?.plannedAt]);
  if (!item) return null;
